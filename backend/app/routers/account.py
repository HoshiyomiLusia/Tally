...
from ..models import Attachment, Budget, Category, Contact, Merchant, PlannedExpense, Position, Transaction, User, Wallet
...
async def reset_my_data(
    ...
    await session.execute(delete(PlannedExpense).where(PlannedExpense.user_id == user.id))  # 审计 #132: 此前重置漏了它
