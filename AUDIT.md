...
- [x] **29** ⏸️[暂缓·07-17: 正确修需"分摊感知的确认流"(确认弹窗按原参与人重建 loan_out 腿),会动到共用的 TransactionForm 加单核心路径,风险大,留作单独专项]🔴 **周期账单「确认扣款」对分摊账单丢 loan_out 应收腿** — `recurring.py:83` 确认时只按 my_share 建支出,原分摊的参与人 loan_out 应收腿没了。→ 每次确认 AA 周期账单,应收凭空缩水。[周期 单命中] **已修(2026-09-05)**: 预测项附带分摊模板(同组借出腿→总额/我的份额/各人份额), 确认扣款按模板预填并走分摊接口并入周期组。
...
# 第三轮彻底检查(2026-09-05 · 数据不变量自检 + 备份往返实测 + 未扫模块审查)

33 项全库不变量 31 项干净; 备份导出→导入往返 10 项全一致。问题集中在功能层, 11 条本轮全部修复。

- [x] **131** **周期账单漏确认一期即从首页预测静默消失**(56 组里 33 组不见了, 含 GitHub/Obsidian/東京ガス 等仍在续费的订阅); 无逾期提示、无停用入口 — 修: 预测改为**自学习**(月度类学"通常每月几号"的中位数、固定天数类学实际间隔中位数, 最近 6 期), 逾期不管多久都露出并标「逾期 N 期」, 超过 12 期默认折叠 + 一键停用; 新增 `POST /recurring/stop/{tid}`(清最新一笔周期天数, 历史保留)。`recurring.py`/`Overview.tsx`
- [x] **132** 备份不含「预定支出」, 导入不清空、重置不清 — 修: export/import/reset 三处接入 PlannedExpense(旧备份无该键→空)。`io.py`/`account.py`
- [x] **133** 单人全额代付(我的份额 0)只有一条借出腿却打了 split_group_id, 被当"分摊腿"锁死不能编辑、账单页无撤销按钮(34 笔) — 修: 创建时不打组; 迁移 0012 清掉存量单腿组的 split_group_id → 变成可编辑的独立借出。`loans.py`
- [x] **134** 老账号缺后加的系统分类(Kyuu/demo 无「投资亏损」), seed 只在注册/重置时跑 — 修: `ensure_system_category` 找不到就按 seed_data 定义补建; 投资结算/对账/核销三处改用它。`internal_cats.py`
- [x] **135** by_month 频率判断只认 `==30/==365` 与预测的区间容差不一致, 含死代码 — 修: 统一用 `_period_months` 容差。
- [x] **136** 导入备份不校验: 未知币种(外键开启后会清库后 500)、交易币种≠钱包币种、转账一腿被跳过留单腿、主币种 — 修: 清库前先校验全部币种与币种一致性(不合法 400 且数据不动); 转账两腿同进退。(含上轮遗留 io 币种校验)
- [x] **137** 删交易只删附件行, 文件成孤儿 — 修: 三条删除路径先收集 stored_name, commit 后 unlink 原图与缩略图。
- [x] **138** 预定支出到期后无提示仍扣减 — 修: 便签到期项红字「已到期, 记完账记得删掉」。
- [x] **139** API 不暴露 attributed_wallet_id(名义归属钱包), 界面看不到 — 修: TransactionRead 暴露; 借贷编辑弹窗提示归属钱包且换钱包会重置。
- [x] **140** created_at 恒为 UTC(SQLite CURRENT_TIMESTAMP)与 JST 的 occurred_on 口径不同 — 修: ORM default=datetime.now 按容器本地时区写入(存量不改)。
- [x] **141** 附件上传只信客户端 MIME; 联系人归档失败无提示; 坏账核销组支出腿显示必被拒的「撤销分摊」死按钮(上轮遗留) — 修: PIL/PDF 头校验不符即删文件 400; 归档 onError; 撤销按钮再排除 contact_id 非空的核销腿。

> 未修(接受): 登录无限速、注册默认开放(家用局域网, 已在 #66 留观察)。

---

# 第四轮交互检查(2026-09-05 · 手机/亮色/键盘/空态 · headless 全页截图)

暗/亮主题 12 页正常、console 零错误、接口 <0.2s; 问题集中在手机布局与键盘交互, 12 条本轮全部修复。

- [x] **142** 账单页悬浮「+」盖住列表最后一行的编辑/删除图标(桌面与手机) — 修: 桌面隐藏(顶部已有添加), 手机列表尾部留出 FAB 高度
- [x] **143** 借贷页账户行手机上竖排挤压 — 修: 标签窄屏独占一行
- [x] **144** 投资页持仓行手机上名字截成 1 字、副标题竖排 — 修: 同上
- [x] **145** 钱包卡片手机双列时名字被 4 个图标挤成「三菱UF…」 — 修: 手机单列全宽
- [x] **146** 「抹除投资」弹层手机上超出右边界 — 修: 窄屏右对齐
- [x] **147** 总分析自定义区间行换行断裂 — 修: 起/止各自成组换行
- [x] **148** 所有弹窗不支持 Esc、无焦点管理 — 修: Modal 加弹窗栈 Esc 只关最上层, 打开时焦点进弹窗(role=dialog); 日期/月份下拉的 Esc 标记已消费不连带关外层
- [x] **149** 记账表单回车无法提交 — 修: 金额框/备注框 Enter 保存(输入法组合中不触发)
- [x] **150** 手机行内图标触控目标偏小 — 修: 账单行按钮 p-2.5、钱包卡图标 p-1.5(桌面不变)
- [x] **151** 记账表单分类 chips 列出系统分类可手选 — 修: 隐藏 4 个系统分类(编辑本就挂系统分类的那笔仍显示)
- [x] **152** 新账号无钱包时记账表单无引导 — 修: Wallet 区显示"先去建钱包"提示与链接
- [x] **153** 无数据时支出节奏画空网格 — 修: 空态文案

## 用户提出的功能补全(2026-09-05)
- [x] **154** AA 分摊账单(房租 + 借出腿)没有铅笔、记错无法改 — 修: 新增 `GET/PUT /loans/split/{group_id}` 整组编辑(总额/我的份额/参与人/钱包/分类/日期/备注); 账单页分摊腿显示铅笔, 表单按整组预填, 分摊勾选锁定
- [x] **155** 投资卖出/买入腿只能换钱包与删除, 记错金额无法改 — 修: 新增 `GET/PUT /investments/sell/{group_id}`(重算盈亏腿、翻转收益/亏损、为 0 删腿、超卖 400)与 `PATCH /investments/buy/{tid}`(期初对同步、缩量守卫); 账单页投资腿铅笔改为完整编辑弹窗
- [x] **156** 同商家短时间多笔不同金额需逐笔录入 — 修: 金额框支持空格/斜杠分隔多金额(如 `30 30 128`)一次建多笔(周期只挂第一笔); 新增「保存并再记一笔」保留钱包/分类/商家只清金额与备注
- [x] **157** 信用卡额度调整没有入口 — 修: 钱包卡片「可用/额度」行改为按钮直达编辑弹窗(未设额度也显示引导); 额度本就在编辑钱包里可改
- [x] **158** 记账表单小分类行只在点大类后才渲染, 点完布局下移、快速点击点错位置 — 修: 用隐形量尺测出各大类小分类行的最高高度作为常驻占位(未选时显示提示文案), 并去掉会改 chip 宽度的 ✓ 前缀与选中加粗, 点大类前后「商家」及各 chip 位置像素级不变(桌面/手机均实测)
- [x] **159** 首页周期账单预测只看未来 7 天, 月中扣款的账单大半个月看不到(用户反馈 8-19 的 pixiv 学出「通常 22 号」→ 9-22 被窗口截掉) — 修: 前瞻窗口改为一个完整月 31 天(后端默认同步), 加「↓ 再往后 31 天」按钮, 标题显示「前 N 天 · 后 M 天」
- [x] **160** 换付款方式 / 用「添加」另起一组 → 老序列永远「逾期 N 期」(房租、OpenAI、Adobe、AWS、Apple One、水道局、Google One 等十几条假逾期) — 修: 预测与分组接口按「商家+分类+币种+种类」(故意不含钱包; 无商家用备注)把时间上前后不重叠的序列串成一条线; 线记住出现过的所有识别键(确认扣款时改了分类/备注老段不脱线); 多条候选按备注相同 > 金额接近 > 结束晚取舍(GitHub Pro/Copilot 各接各的); 半个周期内同额的不接(两张卡各一份); 条目显示「已接上之前 N 段记录」
- [x] **161** 自学习分不清周期误差与漏记/补记 — 修: 统一阈值 10 天; 「最近典型日」在上月/当月/下月三选一(1 号账单 10-31 记归 11-01 期), 季度/半年/年度按锚点对齐周期网格(年费 12-28 扣、1-03 记不再变成 1-28 逾期); 补记签名 = 离典型日远 且 离上一笔正常记录多出一期以上, 补记样本不参与学习并显示「忽略 N 期补记」, 最新一笔是补记时下一期取其后第一个网格日(8-19 账单 9-05 补记 → 9-19 不再跳过); 离典型日远但不是补记 = 扣款日变了, 从这笔往后推且两期内跟上新日; 固定天数类只采纳 [0.6x,1.4x] 的间隔
- [x] **162** 逾期 2 期以上的条目淹没时间轴 — 修: 折叠阈值 12 期 → 2 期(用户要求); 一键停用只针对逾期半年以上且 confirm 列出名单; 全部折叠时空态文案改为「N 个逾期账单已折叠」
- [x] **163** 三轮对抗性审查(共 ~70 代理)附带发现 — 修: 当天到期不算已漏一期(此前刚到期就「逾期 2 期」被折叠); recurrence_period_days 加 1~3660 校验(负数会让逾期循环跑满 240 期), 导入同样过滤, 前端提交前校验且 422 的 detail 数组不再直接渲染(白屏); occurred_on 年份限 1900-2999, 预测对极端年份跳过; 停用接口组内查询过滤 is_recurring 并同时清前端传入那笔; list_groups 与预测同一套串线; 年费/半年补记晚 1.5 个月以上仍归回周期网格(候选窗口扩到 2 个周期); 停订重订/漏期后改扣款日时相隔恰好一期的两笔不再当补记(新节奏两期跟上); 漏几期后一口气补多笔不会把典型日劫持成补记那天; 季度/半年相位整体变了自动重新锚定; 补记判定以上一笔的典型日为基准(上一笔晚记几天不影响); 同商家备注不同(Copilot/Github Pro、Site/Sync)半周期内不串线; 「每月几号」改环形中位数(月末账单跨月记不再被补记劫持)且同日多笔只算一次; 尾部连续 3 笔间隔正常日号一致却与旧典型日差 >10 天 → 直接采纳新节奏
- [x] **164** 首页周期账单未来预测一屏列二十多条 — 修: 只摊开 3 天内的预测, 更远的折叠成一行(展开后才出现「再往后 31 天」), 标题显示实际摊开的范围, 全部折叠时空态文案说明各折叠了几条
- [x] **165** 仪表盘的分类与 Top 商家只有汇总看不到明细 — 修: 大类/小类/商家行都可点, 弹窗按金额从大到小列出当月每一笔(尊重币种筛选); 后端 TransactionFilter 新增 merchant_id 过滤(分类本就含子类)
- [x] **166** 首页三大板块一屏放不下, 每次都要滚很久 — 修: 余额 / 仪表盘 / 周期账单默认折叠成"必要信息"(余额=真实余额+四指标+各币种汇总, 仪表盘=币种+四个 KPI, 周期账单=月度/年度合计对比+待确认与 3 天内预测), 展开状态记在 localStorage; 周期账单折叠时另藏「再往前 7 天」与底部长说明
- [x] **167** 周期账单里「已确认」的过去条目占位 — 修: 已确认默认折叠成一行, 点展开才列出(用户要求)
- [x] **168** 支出节奏图只能在「近 6 / 近 12 个月」两个固定档之间切 — 修: 新增「选月份」浮层可从近 24 个月里任意勾选对照月(预设加了近 3 个月); 图例每个月改成开关, 点一下从对照里去掉、再点加回来, 均值与区间同步重算
- [x] **169** 三个板块的展开开关位置不统一(余额在底部居中, 另两个在标题栏最右) — 修: 统一成"箭头 + 标题"贴在每个板块左上角, 点标题即展开/收起
- [x] **170** 手机上钱包卡片一排一张, 一屏只看得到两三张 — 修: 手机改成一排两张(桌面维持定宽左对齐), 卡内名称与金额字号随之缩小, 名称截断处加 title 提示全名
- [x] **171** 借贷页联系人有多个币种时只有分币种行, 没有这个人的总账 — 修: 联系人卡片在多币种时加一行合计(借出/已还/净额都折算到本位币, 缺汇率单独标注)
- [x] **172** 「待确认」条目仍然铺满周期账单板块 — 修: 待确认合并成一条折叠(原「逾期 2 期以上」单独一条并入其中, 文案里保留逾期数量与「停用逾期半年以上」入口), 与已确认、3 天外预测三条折叠对齐; 折叠后板块只剩合计对比 + 三条折叠 + 近 3 天预测
- [x] **173** 分类 emoji 造型随系统字体变、风格与界面其余部分不一致 — 修: 新增 CategoryIcon 组件, 按分类名映射到 lucide 线描图标(与侧边栏同一套图标库), 颜色按顶级分类分配浅色圆底便于长列表定位; 覆盖仪表盘分类面板、账单行(含转账/借贷/投资种类)、周期账单、记账表单 chips、分类管理页、总分析、报销; 原生 select 里的 emoji 直接去掉; 不动数据库, emoji 字段留作自建分类的回退

# 第五轮彻底检查(2026-09-12 · 接口回归 + 数据体检 + 全页走查)

55 项接口回归全过; 20 项真实数据完整性检查零异常; 9 页 × 明暗 × 手机桌面共 36 次加载零控制台报错。发现并修复 5 条。

- [x] **174** 6 个投资分类(投资/股票/基金/加密货币/债券/其他投资)没有图标映射, 会回退成 emoji — 修: 补上 lucide 映射, 统一 sky 色
- [x] **175** 界面里还有写死的 emoji(📌 预定支出、🟦/🟩 投资买卖、🟥/🟩 借出还款、🌐 全球、➕ 新建) — 修: 换成 lucide 图标或色点, 原生 option 里改用全角符号
- [x] **176** 报销/转移/信用卡还款/投资/借贷 五个表单的钱包 chip 选中时插入 ✓ 会改变宽度、触发整行重排(与 #158 同一类问题) — 修: 去掉 ✓, 选中态已有绿底足够区分
- [x] **177** 账单页「类型」筛选只有支出/收入/借出/还款 4 项, 但列表里存在转账与投资记录, 筛不出来 — 修: 补齐转账转出/转账转入/投资买入/投资卖出
- [x] **178** 「快速添加」面板没走统一 Modal 组件: 暗色主题下面板是白底, 且不能用 Esc 关闭、无焦点管理与 role=dialog — 修: 改用 Modal
- [x] **179** 自建分类若名字没匹配到图标、又没填 emoji, 会显示成空白方块 — 修: 兜底改为中性标签图标(灰色), 占位宽度与正常图标一致
> 另做的检查(未发现问题, 记录备查): 34 项只读接口参数 fuzz(非法月份/越界 limit/颠倒日期/超长查询/不存在 id) 零 5xx; 10 项跨用户越权尝试(改删别人的交易/钱包/分类/商家/联系人、用别人的分类商家记账)全部被拒且未写入任何数据; 导入路径验证(脏周期天数被清成 None、版本不符 400 且不清库); 周期算法单测全通过; 手机端零横向溢出。

## 第五轮的财务正确性核验(2026-09-12, 全部通过, 记录备查)
从原始交易独立重算并与接口逐项对账, 零差异:
- 13 个钱包的 Model A 余额、借出/还款/投出/投回归属(含 attributed_wallet_id) 全部吻合
- JPY/CNY 两个币种的物理余额、信用卡待还、投资中、真实余额全部吻合; 折算合计 968,440 + 2,011,839 = 2,980,279 与接口一致
- 4 个持仓的剩余成本与已实现盈亏(按卖出组重算)全部吻合, 无超卖, 清仓状态正确
- 2 个借贷账户的借出/已还/净额全部吻合
附件端到端通过(上传/下载字节一致/可执行文件与超大文件被拒/跨用户 404/删交易连带删磁盘文件, 零孤儿文件)。
接口耗时: 除全量 5000 条(0.32s)外均在 60ms 内。迁移 head 与库内版本一致(0012)。汇率当日已自动刷新。

## 待你决定(非缺陷)
- [ ] **180** 预算功能是"半死"状态: 前端页面在一次大改版里被删除、`/budgets` 路由重定向到首页, 但后端 router/schema/API 类型与库表都还在, 你名下还有 2 条真实预算(JPY 300,000 生活费 / CNY 200,000 娱乐开销)在界面上完全看不到。要么恢复一个入口, 要么连同后端一起清掉, 我按你说的做。
- [ ] **181** (承 #26 的既定决定, 仅记录新情况) 汇率表是全局共享无 user_id, 这台实例现在有 4 个账号(Lusia/Kyuu/demo/test, 其中 Kyuu 有真实数据)。任何账号手工录入的汇率会影响所有人, 且手工汇率会**永久**阻止该货币对的自动刷新。当前 6272 条全部是 auto, 尚未触发。
- [x] **180** 预算功能恢复入口(按你定的形态: 只有一条总预算 + 一条进度条) — 做法: 后端新增 `GET/PUT /budgets/total`, 一条本位币金额, 本月所有币种支出折算后合并计算(排除对账调整), 返回已花/剩余/百分比/月末推算/缺汇率币种; 写入时把旧的多条预算收敛成一条。前端: 仪表盘 KPI 下一条进度条(折叠态也可见, 细竖线=按天数应有的进度, 超支转红), 设置页一个输入框(留空=不启用)。旧的分类预算 CRUD 接口保留但界面不暴露。你的两条旧预算(JPY 300,000 / CNY 2,000)已按你指示合并为总预算 400,000 JPY, 改前已备份 data/tally.db.20260912-175500.bak
- [x] **182** `user.primary_currency_code` 可为 NULL(注册时不写, 库里 Kyuu 就是 NULL), 任何按本位币聚合的接口对这类账号会 500 — 修: 新增 `resolve_base_currency`(用户设置 > 余额最大的钱包币种 > JPY)
- [x] **183** 设置页「分类」入口副标题仍写着「二级分类树 + emoji」, 与 #173 换成图标后不符 — 修: 改为「二级分类树 · 图标自动匹配」
- [x] **184** 总预算的参照线与月末推算都在做"支出按天数线性"的假设, 对实际数据严重失真 — 你指出支出不可能线性, 核实确实如此: 你过去 6 个月到 12 号平均只走完全月的 26.8%(区间 0%~42.6%, 房租与订阅压在 24-30 号), 线性假设按 40% 算。修: 参照线改为"过去 6 个月同期通常已花多少"(本月 ¥147,981 = 预算的 37%), 月末推算改为 本月已花 + (历史整月均值 - 历史同期均值), 与支出节奏图的"按平均节奏月末约"同一套加法外推, 免得同屏两个数打架; 没有历史的新账号才退回按天数线性并在提示里说明。本月推算由错误的 ¥191,158 变为 ¥481,060
- [x] **185** 项目没有 favicon(只有 PWA 图标, index.html 里没有 `rel="icon"`), 浏览器/工作区侧栏只能显示默认地球; 且原 PWA 图标是黑底加一个几乎看不见的小「T」 — 修: 重做一套"正字计数"图标(tally 本义即计数划线, 四竖一斜, 与记账直接对应), 祖母绿渐变底白线, 出 favicon.svg(矢量, 现代浏览器优先) + favicon.ico(16/32/48/64/128/256 多尺寸) + icon-192/512 + icon-maskable(满幅, 内容收在 80% 安全区) + apple-touch-icon(180 不透明), 并在 index.html 声明 icon 链接与 description
- [x] **186** 账单页筛选钱包后看不到这张卡的余额/额度 — 修: 筛选区下方加一条汇总。选中储蓄卡/电子钱包显示余额(物理口径), 选中信用卡显示待还 + 可用/额度(未设额度会提示), 该钱包上有借出未还或投资中也一并列出; 没选钱包时显示折算到本位币的真实余额 + 物理 + 信用卡待还, 缺汇率的币种会标注
- [x] **187** 切到别的页再回账单页, 筛选条件全部丢失 — 修: 筛选与每页条数存 localStorage, 回来自动恢复(逐字段兜底, 结构变了不会崩); 汇总条右侧给「恢复默认（N）」一键清回, 移动端筛选弹窗里的按钮文案同步
- [x] **188** 账单页筛选的起止日期还是浏览器原生 date 控件, 与应用其它地方的日期选择器不一致 — 修: 改用自带的 DateField; 为筛选场景给它加了 placeholder(空值显示「开始日期 · 不限」)与 clearable(选了之后给 × 清回不限), 空值时隐藏前后一天的箭头(没有基准日, 点了会莫名跳到今天)
- [x] **189** 修 #172 做过头: 你说「待确认也折叠」时我把全部 18 笔待确认并成一条, 结果逾期 1 期、正需要点「确认扣款」的那几笔也被藏了 — 修: 改回只折叠逾期 2 期以上的(那些多半是取消了的订阅), 逾期 1 期的留在时间轴上直接可操作。现在 11 笔折叠、7 笔可见
- [x] **190** 账单页按钱包筛选后点「添加」, 表单仍默认第一个钱包, 每次都要手动改 — 修: TransactionForm 新增 defaultWalletId(只作用于新建, 编辑与确认扣款自带钱包不受影响; 指定的钱包被归档/删掉会退回第一个可用钱包), 账单页把当前筛选的钱包传进去; 币种随钱包自动切换
- [x] **191** 分摊的三个快捷按钮(纯代付 / 我付完余下AA / 全部均摊)原来在参与人列表下方另起一行, 选完人还要往下移一次 — 修: 移到「参与人（除了你自己）」标签行的右侧(那行本来是空的), 选中参与人后原地就能点; 手机 390px 下标签与三个按钮仍在同一行
